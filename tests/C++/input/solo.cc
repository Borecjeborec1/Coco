...
// Platform-specific code to get network interface information
    ...
    static nlohmann::json cpus()
    {
...
#else
        // Read CPU information from /proc/cpuinfo on Linux (example, you may need to adapt)
        ...
        while (std::getline(cpuInfoFile, line))
        {
            if (line.empty())
            {
                // End of one CPU entry, add it to the array
                cpusArray.push_back(cpuInfo);
...
int main(){
  ...
  using os = __os__;
std::cout << std::string("os.EOL:") << os::EOL << '\n';
std::cout << std::string("os.arch:") << os::arch() << '\n';
std::cout << std::string("os.freemem") << JS_substring(JS_toString(os::freemem()), static_cast<double>(0), static_cast<double>(2)) << '\n';
std::cout << std::string("os.homedir:") << os::homedir() << '\n';
std::cout << std::string("os.hostname:") << JS_substring(os::hostname(), static_cast<double>(0), static_cast<double>(1)) << '\n';
std::cout << std::string("os.machine():") << os::machine() << '\n';
std::cout << std::string("os.platform:") << os::platform() << '\n';
std::cout << std::string("os.tmpdir:") << os::tmpdir() << '\n';
std::cout << std::string("os.totalmem:") << os::totalmem() << '\n';
std::cout << std::string("os.type:") << os::type() << '\n';
std::cout << std::string("os.uptime:") << JS_substring(JS_toString(os::uptime()), static_cast<double>(0), static_cast<double>(3)) << '\n';
std::cout << std::string("os.cpus:") << os::cpus() << '\n';
  return 0;
}  
