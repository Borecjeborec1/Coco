...
nlohmann::json JS_flatMap(const nlohmann::json &arr, Callable callback) {
  ...
  for (const auto &element : arr) {
    try {
      ...
      if (mapped.is_array()) {
        // If the result of the callback is an array, concatenate it to the
        // result
        result.insert(result.end(), mapped.begin(), mapped.end());
      } else {
        // If the result is not an array, add it as a single element to the
        // result
...
nlohmann::json JS_map(const nlohmann::json &arr, Callable callback) {
  ...
  for (const auto &element : arr) {
    try {
      // Apply the callback function to each element and add the result to the
      // result array
...
JS_splice(nlohmann::json &arr, int start, int deleteCount,
  ...
  // Copy elements from the original array to the result array, skipping the
  // removed items
