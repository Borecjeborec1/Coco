...
template <typename fpgn, typename hbdj> auto add(fpgn a, hbdj b) {
  return (a + b);
};
...
int main() {
  ...
  auto result = add(static_cast<double>(5), static_cast<double>(3));

  std::cout << result << '\n';
  auto multiply = [](auto a, auto b) { return (a * b); };

  auto product = multiply(static_cast<double>(2), static_cast<double>(4));

  std::cout << product << '\n';
  auto divide = [](auto a, auto b) { return (a / b); };

  auto quotient = divide(static_cast<double>(10), static_cast<double>(2));
